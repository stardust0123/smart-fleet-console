    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
          <div><p className="text-sm font-medium text-gray-500">Total Fleet Vehicles</p><h4 className="text-2xl font-bold text-gray-800 mt-1">{stats?.totalVehicles || 0}</h4></div>
          <div className="p-3 bg-blue-50 text-blue-600 rounded-xl"><Truck className="h-6 w-6" /></div>
        </div>
        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
          <div><p className="text-sm font-medium text-gray-500">Open Maintenance Jobs</p><h4 className="text-2xl font-bold text-gray-800 mt-1">{stats?.openJobs || 0}</h4></div>
          <div className="p-3 bg-amber-50 text-amber-600 rounded-xl"><Wrench className="h-6 w-6" /></div>
        </div>
        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
          <div><p className="text-sm font-medium text-gray-500">Predictive Alerts</p><h4 className="text-2xl font-bold text-gray-800 mt-1">{stats?.predictiveAlerts || 0}</h4></div>
          <div className="p-3 bg-red-50 text-red-600 rounded-xl"><AlertTriangle className="h-6 w-6" /></div>
        </div>
        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
          <div><p className="text-sm font-medium text-gray-500">Active Mechanics</p><h4 className="text-2xl font-bold text-gray-800 mt-1">{stats?.mechanicsAvailable || 0}</h4></div>
          <div className="p-3 bg-green-50 text-green-600 rounded-xl"><Users className="h-6 w-6" /></div>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm"><h3 className="text-base font-semibold text-gray-800 mb-4">Workshop Workload Distribution</h3><WorkshopWorkloadChart data={workload} /></div>
        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm"><h3 className="text-base font-semibold text-gray-800 mb-4">Top Predictive Alert Types</h3><PredictiveAlertChart data={alertTypes} /></div>
        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm"><h3 className="text-base font-semibold text-gray-800 mb-4">Top Vehicles Generating Alerts</h3><TopVehiclesAlertChart data={topVehicles} /></div>
        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm"><h3 className="text-base font-semibold text-gray-800 mb-4">Maintenance Cost & Downtime by Model</h3><ManagerCostChart data={costAndDowntime} /></div>
      </div>
        ...
        <div className="overflow-x-auto max-h-96">
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {filteredMechanics && filteredMechanics.length > 0 ? (
                filteredMechanics.map((mech: any, idx: number) => (
                  <tr key={idx} className="border-b hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3 font-medium text-gray-800">{mech.mechanic_id}</td><td className="px-4 py-3 text-gray-700">{mech.full_name}</td>
              ...
              ) : (<tr><td colSpan={4} className="px-4 py-8 text-center text-gray-500">No available mechanics found.</td></tr>)}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden p-4">
          <h3 className="text-base font-semibold text-gray-800 mb-4">Urgent Immediate Repairs</h3>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden p-4 flex flex-col">
          <h3 className="text-base font-semibold text-gray-800 mb-4">Inventory Alerts & Supplier Deviations</h3>
          ...
          <div className="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-lg shadow-sm">
            ...
            <form action={updatePartStock} className="flex flex-col sm:flex-row gap-3 items-end">
              <div className="flex-1 w-full">
                <label className="text-xs text-gray-600 font-medium block mb-1">Select Part to Update</label>
              ...
              <div className="w-full sm:w-32">
                <label className="text-xs text-gray-600 font-medium block mb-1">New Quantity</label>
